...
GOOGLE_API_KEY = "AIzaSyDxr-W59D75X1OGCgFCBdtNPyE2ElyTwP8"
...
def reset_interview():
    ...
    speak('Interview session ended.')

    while True:
        try:
            with source as s:
                audio = r.listen(s)
                response_text = wav_to_text('response.wav').strip().lower()
...
def start_listening():
    ...
    print('You can start speaking now.Please state the job title to begin the interview.')
    speak('You can start speaking now.Please state the job title to begin the interview.')
